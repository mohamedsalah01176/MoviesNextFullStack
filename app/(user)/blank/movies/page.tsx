...
type IAction={
  ...
  const Page = () => {
    ...
    useEffect(()=>{
      (async()=>{
          const results =await(await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/movies`,{
          cache: 'no-store'
        })).json();
        setMovies(results)
      })()
  ...
  // update movie
    ...
    useEffect(()=>{
      if(movie){
        ...
        dispatich({type:"input",field:"rate",value:movie?.rate || ""})
  ...
  return (
    <div className='min-h-[98vh] bg-[#161a1e] pb-20'>
      ...
      {movieId >0 &&
      <div className='bg-[#161a1e]  fixed top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] w-[70%] rounded-2xl '>
